...
class Country():
    def __init__(self, country_code):
        ...
        self.pop_grid = gpd.read_file(f"{DATA_PATH}JRC_GRID_2018/JRC_POPULATION_2018.shp")
        self.country_code = country_code

        # get relevant data
        self.URBA = self.URBA if self.country_code=="EU" else self.URBA.loc[self.URBA['CNTR_CODE']==self.country_code]
        self.pop_grid = self.pop_grid if self.country_code=="EU" else self.pop_grid.loc[self.pop_grid['CNTR_ID']==self.country_code]
    ...
    def compute_population_per_lau(self):
        ...
        urba_grid_overlay_areas = urba_grid_overlay.merge(self.pop_grid[['OBJECTID', 'TOT_P_2018', 'area']], left_on='OBJECTID_2', right_on='OBJECTID', how='left')
        ...
        overall_grid_areas = urba_grid_overlay_areas[["OBJECTID", 'overlay_area', 'area']].groupby('OBJECTID').agg({'overlay_area':'sum'}).rename(columns={'overlay_area':'total_square_area'})
        ...
        self.weighted_pop_lau = urba_grid_overlay_areas[['OBJECTID_1', 'fraction_pop']].groupby('OBJECTID_1').sum()
        self.URBA = self.URBA.merge(self.weighted_pop_lau, left_on='OBJECTID', right_index=True)
    ...
    def print_summary_DGURBA(self, savefig=False):
        ...
        ax = self.DGURBA_summary['population', 'sum'].plot.bar(color=c1, width=width, position=1, rot=0)
        ax2 = ax.twinx()
        self.DGURBA_summary['area_pct', 'sum'].plot.bar(color=c2, ax=ax2, width=width, position=0, rot=0)
        plt.xlim([-1, 3])
    ...
    def get_summary_DGURBA(self, plot=True):
        ...
        aggregations = {'population': ['sum', 'mean', 'idxmax', 'idxmin'],'area_pct': ['sum', 'mean', 'idxmax', 'idxmin']}
